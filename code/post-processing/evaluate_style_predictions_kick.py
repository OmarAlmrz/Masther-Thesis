...
label_names = ['Null', 'Front Crawl', 'Breaststroke', 'Backstroke', 'Butterfly', "Kick"]
label_names_abb = ['Null', 'Fr', 'Br', 'Ba', 'Bu', "kc"]

loso_path = r'.\results\with_kick\7' #Path where trained LOSO models are stored
save_path = 'SAVE_PATH'
...
cm_ix = {1: 0, 2: 1, 3: 2, 4: 3, 6: 4, 'mixed': 5}
cm = np.zeros((6, 6))
...
print(utils.write_confusion_matrix(cm_norm, labels=[1, 2, 3, 4, 6, 0]))
print(utils.write_confusion_matrix(cm, labels=[1, 2, 3, 4, 6, 0]))
print(utils.write_latex_confmat(cm, labels=['Front Crawl', 'Breaststroke', 'Backstroke', 'Butterfly', "Kick" ,'Mixed'], is_integer=True))
print(np.mean(np.diag(cm_norm)))

expanded_matrix = utils.expand_confusion_matrix(cm)
print(utils.write_confusion_matrix_expanded(expanded_matrix, labels=[1, 2, 3, 4, 6, 0]))