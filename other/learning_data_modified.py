...
class LearningData(object):
    ...
    def load_data(self, data_path, data_columns, labels, users=None, avoid_names = None):
        """
        ...
        for user in self.users:
            ...
            csv_in_path = self._filter_data(avoid_names, csv_in_path)
    ...
    def _filter_data(self, avoid_label, files):
        if avoid_label is None: return files
        return [f for f in files if f.split('_')[0] not in avoid_label]
    ...
    def compile_windows(self, norm_type='statistical', label_type='proportional', multi_class_thresh=0.2,
        ...
        for group in groups:
            for user in self.users:
                ...
                for label, value in cnt_user_label.items():
                    if value == 0 and label not in to_remove:
                        to_remove.append(label)
                        # temp_user_windows.pop(label, None)
                        # self.data_windows[group].pop(label, None)
        ...
        # Remove labels that are not present in any user
        # available_labels = list(self.data_windows["original"].keys())
        # to_remove = [label for label in self.labels if label not in available_labels]
        
        # for label in to_remove:
        #     self.labels.remove(label)
        #     self.labels_index.pop(label, None)
    ...
    def batch_generator_dicts(self, train_dict, batch_size=64, noise_std=None, mirror_prob=None, random_rot_deg=30):
        ...
        available_labels = list(train_dict.keys())
        while True:
            ...
            for i in range(batch_size):
                ...
                r_label = np.random.choice(available_labels)
    ...
    def draw_train_val_dicts(self, users_train, users_per_class=3, validation = True):
        """
        ...
        if validation:
            ...
            train_dict = {}
            for l in self.data_windows['original'].keys() :
                users_in_class = list(self.data_windows['original'][l].keys())
                users_train_in_class = [u for u in users_in_class if u in users_train]
                if users_train_in_class:    
                    train_dict[l] = users_train_in_class
